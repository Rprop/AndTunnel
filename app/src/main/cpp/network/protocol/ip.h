...
struct ipv4_hdr {
#if BYTE_ORDER == LITTLE_ENDIAN
    uint8_t hdrlen: 4; // hdrlen * 4 == sizeof(ipv4_hdr)
    uint8_t version: 4;
#else
    uint8_t  version : 4;
    uint8_t  hdrlen : 4;
...
#include <endian.h> // for __swap16

/*
 *	Helper macros
 */
#define DOT_IPV4_U(us)        (us)[0], (us)[1], (us)[2], (us)[3]
#define DOT_IPV4(s)           DOT_IPV4_U((const unsigned char *)s)
#define DOT_IPV4_PORT(s, p)   DOT_IPV4(s), __swap16(p)
#define DOT_IPV4_FORMAT       "%u.%u.%u.%u"
#define DOT_IPV4_PORT_FORMAT  DOT_IPV4_FORMAT ":%u"